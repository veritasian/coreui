...
async function statusFor(type) {
  if (type === 'llama.cpp') {
    ...
    return { installed: false, available: false, note: '未安装 llama.cpp，请双击引擎安装包' };
  }
  // swiftlm
  ...
  return { installed: false, available: false, note: '未安装 SwiftLM，请双击引擎安装包（仅 Apple Silicon）' };
}
...
export async function testEngine({ type = 'llama.cpp' } = {}) {
  ...
  if (!probe.launched) {
    return {
      ok: false,
      executable,
      msg: `引擎无法启动：${probe.error || '未知错误'}（可能被隔离或权限不足，请重新双击引擎安装包）`,
      path: bin,
    };
  }
  if (type === 'llama.cpp') {
